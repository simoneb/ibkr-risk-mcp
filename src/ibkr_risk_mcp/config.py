...
#: How this server is *served*, as opposed to what it talks to: transport,
#: listen address, and the bearer-token boundary in front of it. Kept separate
#: from Settings above because the two describe different machines — the
#: IBKR_MCP_ prefix exists so that this server's host and port stay visibly
#: distinct from TWS's, which are IBKR_HOST and IBKR_PORT.
remote = load_remote_settings("IBKR_MCP_")
